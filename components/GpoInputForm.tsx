    ...
    <div className="bg-black/20 backdrop-filter backdrop-blur-lg rounded-xl border border-white/10 shadow-2xl p-6 h-full flex flex-col">
      <div className="mb-4">
        ...
        <p className="text-gray-400 text-sm">
          Paste the content of your GPO reports (XML or HTML) below, or drag & drop files into the text boxes.
        </p>
      </div>
        ...
        <div className="space-y-6 flex-grow">
          {inputs.map((input, index) => (
            <div key={input.id} className="relative">
              <label className="block text-sm font-medium text-gray-300 mb-2">GPO Report #{index + 1}</label>
              <textarea
                value={input.content}
        ...
        <div className="mt-4 text-center">
            ...
            {isSizeExceeded && (
                <p className="text-red-400 text-xs mt-1">
                    Total size of reports exceeds the 25MB limit. Please remove or shorten some reports.
                </p>
            )}
        </div>
