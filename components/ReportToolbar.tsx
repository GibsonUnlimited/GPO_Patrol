...
export const ReportToolbar: React.FC<ReportToolbarProps> = ({ analysis, script, onSaveSession, onClearSession }) => {
    ...
    const handleExportSessionJson = () => {
        ...
        link.href = url;
        link.download = `Carlisle_Modernization_Audit_${new Date().toISOString().slice(0, 10)}.json`;
    ...
    const handleExportDetailedFindingsCsv = () => {
        ...
        link.setAttribute("download", "carlisle_findings.csv");
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
    };
    ...
    const handleExportConsolidationCsv = () => {
        ...
        const link = document.createElement("a");
        link.setAttribute("href", encodedUri);
        link.setAttribute("download", "carlisle_consolidation_report.csv");
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
    };
    ...
    const handleExportPdf = () => {
        ...
        const addPageHeaderAndFooter = (docInstance: any) => {
            ...
            for (let i = 1; i <= pageCount; i++) {
                ...
                docInstance.text('Carlisle Policy Control - Forensic Report', pageMargin, 10);
        ...
        doc.text("Forensic Policy Audit", pageWidth / 2, pageHeight / 2 - 20, { align: 'center' });
        doc.setFontSize(14);
        doc.setFont('helvetica', 'normal');
        doc.text(`Platform: Carlisle Policy Control`, pageWidth / 2, pageHeight / 2 - 10, { align: 'center' });
        doc.text(`Generated on: ${new Date().toLocaleString()}`, pageWidth / 2, pageHeight / 2 - 2, { align: 'center' });
        ...
        doc.save("carlisle_forensic_audit.pdf");
    };
    ...
    return (
        <div className="bg-black/20 backdrop-filter backdrop-blur-lg rounded-xl border border-white/10 shadow-2xl p-3 flex flex-wrap justify-between items-center gap-4">
            <h2 className="text-lg font-bold text-gray-200">Roadmap Audit & Save Operations</h2>
            <div className="flex items-center space-x-2 flex-wrap gap-2">
                 <button onClick={onClearSession} title="Clear current session" className="p-2 text-gray-400 hover:text-red-400 rounded-md hover:bg-gray-700 transition-colors">
                    <TrashIcon className="w-5 h-5" />
                 </button>
                <button 
                    onClick={handleSave} 
                    disabled={saveStatus === 'saved'}
                    className={`inline-flex items-center px-3 py-1.5 text-sm font-medium rounded-md transition-all shadow-lg ${
                        saveStatus === 'saved'
                ...
                >
                    {saveStatus === 'saved' ? (
                        <><CheckIcon className="w-5 h-5 mr-2" /> Forensic Entry Locked</>
                    ) : (
                ...
                <div className="relative">
                    <button 
                        ...
                        className="inline-flex items-center px-3 py-1.5 text-sm font-medium rounded-md bg-gray-700 hover:bg-gray-600 text-white transition-colors"
                    >
                 ...
                 <button onClick={handleExportPdf} className="inline-flex items-center px-3 py-1.5 text-sm font-medium rounded-md bg-gray-700 hover:bg-gray-600 text-gray-200 transition-colors">
                    <PdfIcon className="w-5 h-5 mr-2" /> Forensic PDF
                </button>
            </div>
        </div>
    );
